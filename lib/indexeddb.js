...
Indexed.prototype.between = transaction('readonly', function(store, tr, lower, upper, includeLower, includeUpper, cb){
  var result = [];

  if(typeof lower === 'function')
    cb = lower; lower = void 0;

  if(typeof upper === 'function')
    cb = upper; upper = void 0;

  if(typeof includeLower === 'function')
    cb = includeLower; includeLower = void 0;

  if(typeof includeUpper === 'function')
    cb = includeUpper; includeUpper = void 0;
  ...
  request(store.openCursor(range), function(err){
    var cursor = this.result;
    if(!cursor) return cb(null, result);
    result.push(cursor.value);
    cursor['continue']();
  });
});
...
Indexed.prototype.range = function(options){
  var only  = options.only
    , lower = options.lowerBound
    , upper = options.upperBound
    , includeLower = typeof options.includeLower === 'boolean' ? options.includeLower : true
    , includeUpper = typeof options.includeUpper === 'boolean' ? options.includeUpper : true

  if(only)
    return IDBKeyRange.only(only);

  if(lowerBound && !upperBound)
    return IDBKeyRange.lowerBound(lowerBound, !includeLower);

  if(!lowerBound && upperBound)
    return IDBKeyRange.upperBound(upperBound, !includeUpper);

  if(lowerBound && upperBound)
    return IDBKeyRange.bound(lowerBound, upperBound, !includeLower, !includeUpper);
}
