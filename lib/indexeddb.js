...
var LevelDown = require('./abstract/abstract-leveldown').AbstractLevelDOWN;
...
Indexed.prototype._put = transaction('readwrite', function(store, tr, key, val, options, cb) {
  ...
  if (buffer.isBuffer(val)) val = buffer.bufferToStr(val);
...
Indexed.prototype._batch = transaction('readwrite', function(store, tr, operations, options, cb) {
  cb();
});

/**
 * Access to browser Buffer implementation
 *
 * @options {Any} buffer
 * @api private
 */
...
Indexed.prototype._checkKeyValue = function(key, keyValueType) {
  if (key === null || key === undefined)
    return new Error(keyValueType + ' cannot be `null` or `undefined`');
  if (buffer.isEmpty(key))
    return new Error(keyValueType + ' cannot be an empty ArrayBuffer');
  if (key === '')
    return new Error(keyValueType + ' cannot be an empty String');
  if (type(key) === 'array' && key.length === 0)
    return new Error(keyValueType + ' cannot be an empty Array'); // TODO: abstract typo
};
...
Indexed.prototype._upgrade = function(db, cb) {
  ...
  if (config.action && !that.openOptions.createIfMissing)
