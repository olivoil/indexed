...
Indexed.supported = (function() {
  var IDBDatabase       = window && (window.IDBDatabase || window.webkitIDBDatabase);
  var IDBTransaction    = window && (window.IDBTransaction || window.webkitIDBTransaction);
...
Indexed.prototype.get = transaction('readonly', function(store, tr, key, val, cb) {
  if('function' == type(val)){
    cb = val; val = key; key = null;
  }
  ...
  if(key){
    ...
    if(index.unique){
      request(index.get(val), function(err) {
        cb(err, this.result);
      });
    } else {
      var range = IDBKeyRange.only(val);
      var result = [];

      request(index.openCursor(range), function(err){
        var cursor = this.result;
        if(!cursor) return cb(undefined, result);
        result.push(cursor.value);
        cursor['continue']();
      });
    }
    ...
    request(store.get(val), function(err) {
      cb(err, this.result);
    });
  }
});
...
Indexed.prototype.createIndex = function(name, prop, opts){
  var self = this;
  ...
  this.indexes.push([name, prop, opts]);
}
...
Indexed.prototype._upgrade = function(db, cb) {
  ...
  req.onupgradeneeded = function(event) {
    ...
    if (config.action) {
      var store = this.result.createObjectStore(that.name, { keyPath: that.key });
